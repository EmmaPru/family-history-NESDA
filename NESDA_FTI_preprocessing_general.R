...
# Purpose: This script preprocesses the NESDA Family Tree Inventory (FTI) data. 
# It reads in the NESDA data, tidies it, and extracts relevant information for the FTI project. 
# The script also diagnoses depression based on the FTI questionnaire items.
...
fti_raw %>%
  select(ends_with("v01")) %>%              # select relevant columns
  pivot_longer(everything(),               # turn wide to long format
               names_to = "variable", 
               values_to = "value") %>%
  filter(value %in% c(1, 2, 3)) %>%         # keep only 1, 2, 3
  count(value)                              # count occurrences

fti_raw %>%
  select(ends_with("v02")) %>%              # select relevant columns
  pivot_longer(everything(),               # turn wide to long format
               names_to = "variable", 
               values_to = "value") %
  filter(value %in% c(1, 2, 3)) %>%         # keep only 1, 2, 3
...
sum(count)    
    ...
    # This dataframe has one hell of a lot of columns. And it consists mostly of NAs.
# We are going to make it into a 'tidy' long dataframe, that is perfect for working in the tidyverse. We want to have only one observation per row - columns should then be pident, questionnaire item, value.
...
# turn all answers into characters, then pivot to long, then remove redundant part of question label
fti <- fti_raw |> 
  mutate(
...
fti_dbl |> 
  group_by(question) |> 
  summarise(mean = mean(answer, na.rm = TRUE))



# How many ppt did not complete the questions?

# which participants have less than 3 non-NAs?
fti_dbl |> 
  group_by(pident) |> 
...
# now we are going to remove them from the other dataframe, because they are annoying
...
# Let's look at the birth years for a minute
g01 <- fti_existing |> 
  filter(question == "g01") |> 
...
write.csv(age, file = here("Data", "FTI_age.csv"))
