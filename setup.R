...
# -
# Install PAFGRS from github if needed
...
packages <- c(
  ...
  , "haven",
  "here", 
  "locfit", ## for smoothing with locfit)
  "lmtest"
...
# Function to compute AUC and AUC.SE for a combined model
assess_performance_flexibly_covariates <- function(data, coding_type = "DEP_simple_with_ANX_simple",
                                        outcome = "DEP_index", predictors = c("PAFGRSplus", "PRS")) {
  df <- data |>
    filter(coding == coding_type) |>
    pivot_wider(names_from = indicator, values_from = value) |>
    mutate(Sex = as.factor(Sexe), Age = as.numeric(Age), EA = as.numeric(aedu))

  formula_str <- paste(outcome, "~", paste(predictors, collapse = " + "), "+ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + Age + Sex + EA")
  model <- glm(as.formula(formula_str), data = df, family = "binomial")
  pred_probs <- predict(model, type = "response")
  ...
  return(list(results = results, model = model, roc_obj = roc_obj, summary = summary(model)))
}
...
plot_AUC <- function(df, groups, title = NULL) {
  # Function to generate AUC plot with error bars
  # Input: df (data frame with AUC and SE), title (title of the plot as string)
  ggplot(data = df) +
    aes(x = groups, y = AUC) +
    geom_errorbar(aes(x = groups, ymin = AUC - SE, ymax = AUC + SE), colour = "#35A0ABFF", width = .2, position = position_nudge(0), size = 1) +
...
# Function by Pietro to perform quantile-wise logistic regression
